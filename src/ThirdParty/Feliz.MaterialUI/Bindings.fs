...
type Breakpoints =
  ...
  /// the first argument and less than the screen size given by the breakpoint key in
  /// the second argument.
  abstract between: start: MuiBreakpointKey * ``end``: MuiBreakpointKey -> string

// type Spacing =
//       abstract m: n: int: IThemeProp = unbox ("spacing.m", n)
//       static member inline mt (n: int) : IThemeProp = unbox ("spacing.margin-top", n)
//       static member inline mr (n: int) : IThemeProp = unbox ("spacing.margin-right", n)
//       static member inline mb (n: int) : IThemeProp = unbox ("spacing.margin-bottom", n)
...
module Extensions =
  type Breakpoints with
    ...
    /// screen size.
    member inline this.betweenXsSm =
      this.between (MuiBreakpointKey.Xs, MuiBreakpointKey.Sm)

    /// Returns a media query string ready to be used with most styling solutions, which
    /// matches screen widths greater than the `xs` screen size and less than the the `md`
    /// screen size.
    member inline this.betweenXsMd =
      this.between (MuiBreakpointKey.Xs, MuiBreakpointKey.Md)

    /// Returns a media query string ready to be used with most styling solutions, which
    /// matches screen widths greater than the `xs` screen size and less than the the `lg`
    /// screen size.
    member inline this.betweenXsLg =
      this.between (MuiBreakpointKey.Xs, MuiBreakpointKey.Lg)

    /// Returns a media query string ready to be used with most styling solutions, which
    /// matches screen widths greater than the `xs` screen size and less than the the `xl`
    /// screen size.
    member inline this.betweenXsXl =
      this.between (MuiBreakpointKey.Xs, MuiBreakpointKey.Xl)

    /// Returns a media query string ready to be used with most styling solutions, which
    /// matches screen widths greater than the `sm` screen size and less than the the `md`
    /// screen size.
    member inline this.betweenSmMd =
      this.between (MuiBreakpointKey.Sm, MuiBreakpointKey.Md)

    /// Returns a media query string ready to be used with most styling solutions, which
    /// matches screen widths greater than the `sm` screen size and less than the the `lg`
    /// screen size.
    member inline this.betweenSmLg =
      this.between (MuiBreakpointKey.Sm, MuiBreakpointKey.Lg)

    /// Returns a media query string ready to be used with most styling solutions, which
    /// matches screen widths greater than the `sm` screen size and less than the the `xl`
    /// screen size.
    member inline this.betweenSmXl =
      this.between (MuiBreakpointKey.Sm, MuiBreakpointKey.Xl)

    /// Returns a media query string ready to be used with most styling solutions, which
    /// matches screen widths greater than the `md` screen size and less than the the `lg`
    /// screen size.
    member inline this.betweenMdLg =
      this.between (MuiBreakpointKey.Md, MuiBreakpointKey.Lg)

    /// Returns a media query string ready to be used with most styling solutions, which
    /// matches screen widths greater than the `md` screen size and less than the the `xl`
    /// screen size.
    member inline this.betweenMdXl =
      this.between (MuiBreakpointKey.Md, MuiBreakpointKey.Xl)

    /// Returns a media query string ready to be used with most styling solutions, which
    /// matches screen widths greater than the `lg` screen size and less than the the `xl`
    /// screen size.
    member inline this.betweenLgXl =
      this.between (MuiBreakpointKey.Lg, MuiBreakpointKey.Xl)
...
type ComponentVariant = { props: seq<IReactProperty>; style: seq<IStyleAttribute> }
...
type TransitionPropsType =
  inherit PropsObject
  abstract in': bool
  abstract onEnter: obj -> unit
  abstract onExited: obj -> unit

type PopperTransitionProps =
  abstract TransitionProps: TransitionPropsType
